...
$hq = get_str('order', 20);
$hub = $tab === 'hub' ? db_all('SELECT * FROM hub_log' . ($hq !== '' ? ' WHERE order_code = ?' : '') . ' ORDER BY id DESC LIMIT 150', $hq !== '' ? [strtoupper($hq)] : []) : [];
...
<div class="panel"><div class="panel-header">PAYMENTS (PAYMENT HUB TRANSACTIONS)</div><div class="panel-body">
    ...
    <?php if ($tab === 'hub') { ?>
        ...
        <div class="admin-bar"><form method="get"><input type="hidden" name="tab" value="hub"><input type="text" name="order" placeholder="Order ID, e.g. DOC-AB12CD34" value="<?= e($hq) ?>"><button class="btn-classic primary btn-sm" type="submit">FILTER</button></form></div>
        <div class="gv-wrap"><table class="gv-table compact"><thead><tr><th>When</th><th>Call</th><th>Order</th><th>HTTP</th><th>Time</th><th>Hub replied</th></tr></thead><tbody>
