...
$rows = db_all('SELECT r.*, d.title, o.order_code FROM document_reviews r JOIN documents d ON d.id = r.document_id JOIN orders o ON o.id = r.order_id
...
<div class="panel"><div class="panel-header">VERIFIED-BUYER REVIEWS</div><div class="panel-body">
    ...
    <div class="gv-wrap"><table class="gv-table compact"><thead><tr><th>Document</th><th>Rating</th><th>Review</th><th>Order</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>
    <?php foreach ($rows as $r) {
        echo '<tr><td class="doc-title"><a href="edit-document.php?id=' . (int)$r['document_id'] . '">' . e($r['title']) . '</a></td><td class="nowrap">' . stars_html((float)$r['rating']) . '</td>'
            . '<td><strong>' . e($r['name']) . '</strong><span class="sub">' . e($r['comment'] ?: '—') . '</span></td><td class="mono"><a href="orders.php?q=' . e(rawurlencode($r['order_code'])) . '">' . e($r['order_code']) . '</a></td>'
