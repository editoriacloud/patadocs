...
if (!$order || !order_key_ok($order, get_str('k', 64))) { json_out(['ok' => false, 'status' => 'unknown', 'message' => 'Order not found.'], 404); }
$order = order_refresh($order);
switch ($order['status']) {
    case 'paid':
        json_out(['ok' => true, 'status' => 'paid', 'order' => $order['order_code'], 'receipt' => $order['mpesa_receipt'], 'downloads' => order_download_links($order)]);
    ...
    default:         json_out(['ok' => true, 'status' => 'pending']);
}
